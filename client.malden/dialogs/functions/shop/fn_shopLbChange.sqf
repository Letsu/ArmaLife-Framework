...
_dataArr    = _data splitString ",";
 ...
 _index     = call compile (_dataArr select 1); //Change to parseSimpleArray
...
_buyPrice    = _itemArr select 2;
_sellPrice   = _itemArr select 3;
_conditon    = _itemArr select 4;

//Set Text of the Price Label to Price of cur. selected Item.
_text = format ["%1; %2", _buyPrice, _sellPrice];
